...
contract Betting {
    ...
    function settleGroup(uint256 groupId) public {
        // check that the last game of the groups bet slip has been played

        // find every bet slip that corresponds to the group
        uint256[] memory betSlipIds = betSlipsByGroup[groupId];

        // fetch the results of each match in the bet slip using chainlink
